...
import { useDriverManageOrdersStore } from "./ManageOrders.zustand";

const ModalConfirmOrder = ({ order }: { order: OrderType }) => {
  const { actionCloseModal } = useModalStore();
  const { actionAcceptOrder } = useDriverManageOrdersStore();
  const handleAcceptOrder = React.useCallback(() => {
    socket.emit("driver-accepted-order", order);
...
const ManageOrders = () => {
  ...
  const { orders, actionHasNewOrder } = useDriverManageOrdersStore();
  React.useEffect(() => {
    socket.on("wait-driver-accept-order", (order: OrderType) => {
